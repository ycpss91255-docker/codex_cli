...
對話記錄與 Session 資料透過 bind mount 持久化儲存，容器重啟後仍可保留。
...
自動產生的 `.env` 檔案控制所有建置與執行時參數。詳情請參閱 [.env.example](.env.example)。
...
測試項目包含：Codex CLI 是否可用、開發工具、系統設定（非 root 用戶、時區、語系）、其他 AI CLI 工具的不存在（claude、gemini），以及不必要工具的不存在（tmux、vim、fzf、terminator）。

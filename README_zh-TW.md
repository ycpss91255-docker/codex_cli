# Codex CLI Docker 開發環境

內建 OpenAI Codex CLI 的 Docker-in-Docker (DinD) 開發容器，提供 CPU 與 NVIDIA GPU 兩種版本。以非 root 使用者執行，UID/GID 對應宿主機。
...
- 隔離的 Docker-in-Docker 容器，內建 OpenAI Codex CLI
- 非 root 使用者，自動偵測宿主機 UID/GID
- OAuth 憑證首次啟動自動複製，對話紀錄持久化於本地
- 可選 GPG AES-256 加密 API keys
...
- 宿主機上先完成 Codex CLI（`codex`）的 OAuth 登入
...
../agent_myproject/
└── .codex/     # Codex CLI 對話紀錄、設定、session
...
憑證（`~/.codex`）以唯讀方式掛載進容器，首次啟動時複製到資料目錄。後續啟動直接使用既有資料。
...
測試內容：Codex CLI 可用性、開發工具、系統設定（非 root 使用者、時區、語系），確認其他 AI CLI 未安裝（claude、gemini），以及確認不必要的工具未安裝（tmux、vim、fzf、terminator）。
...
| `devel` | Codex CLI、entrypoint、切換至非 root 使用者 |
